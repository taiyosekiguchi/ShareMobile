...
| アルゴリズム | 精度 | 速度 | iOS対応 | 備考 |
|---|---|---|---|---|
| **CREPE** | ★★★★★ | 低（DL推論） | Core MLに変換可能 | CNNベース、最高精度、iKalaデータセットで90.5% |
| **pYIN** | ★★★★☆ | 中 | aubio経由 | 確率的YIN、iKalaで91%精度、MIR研究の標準 |
| **YIN** | ★★★☆☆ | 高 | aubio-iOS-SDK | 古典的手法、リアルタイム向き |
| **Aubio** | ★★★☆☆ | 高 | CocoaPods対応済み | 複数アルゴリズム統合ライブラリ |

**推奨**: サーバーサイドでは **CREPE**（精度最優先）、オンデバイス前処理では **pYIN via aubio**

#### CREPEの特徴
- 時間領域波形を直接入力とするCNN
- 出力: 時刻・周波数・信頼度のベクトル列
- 信頼度の低いフレームを除去することでクリーンなピッチ系列を得る
- 50ms間隔でサンプリング
...
**FastDTW**: O(N²) → O(N) に高速化。1万曲規模でも実用的。
